...
export default function SignalsTab() {
  ...
  const [filter, setFilter] = useState<"confirmed" | "pending" | "watchlist">("confirmed");

  const tickers = Object.keys(data);
  const confirmed = tickers.filter(t => data[t].confirmed && data[t].score >= 70).sort((a, b) => data[b].score - data[a].score);
  const pending = tickers.filter(t => data[t].pending && data[t].score >= 60 && !data[t].confirmed).sort((a, b) => data[b].score - data[a].score);
  const watchlist = tickers.filter(t => data[t].score >= 55 && !data[t].confirmed && !data[t].pending).sort((a, b) => data[b].score - data[a].score);

  const lists = { confirmed, pending, watchlist };
