...
export default function ArchiveTab() {
  ...
  const [sinyalLoading, setSinyalLoading] = useState(false);
  ...
  // Özet istatistikler
  const allRecords = Object.values(sinyalArsiv);
  const stats = {
    toplam: allRecords.length,
    hedef: allRecords.filter(r => r.durum === 'HEDEF TUTTU').length,
    stop: allRecords.filter(r => r.durum === 'STOP LOSS').length,
    acik: allRecords.filter(r => r.durum === 'AÇIK').length,
    kar: allRecords.filter(r => r.durum === 'AÇIK' && (r as any).kar_bolgesi_aktif).length,
    risk: allRecords.filter(r => r.durum === 'AÇIK' && (r as any).risk_bolgesi_aktif).length,
  };
  const basariOrani = stats.toplam > 0 ? ((stats.hedef / stats.toplam) * 100).toFixed(1) : '0.0';
  const filteredRecords = activeFilter
    ? allRecords.filter(r => {
        if (activeFilter === 'toplam') return true;
        if (activeFilter === 'hedef') return r.durum === 'HEDEF TUTTU';
        if (activeFilter === 'stop') return r.durum === 'STOP LOSS';
        if (activeFilter === 'acik') return r.durum === 'AÇIK';
        if (activeFilter === 'kar') return r.durum === 'AÇIK' && (r as any).kar_bolgesi_aktif;
        if (activeFilter === 'risk') return r.durum === 'AÇIK' && (r as any).risk_bolgesi_aktif;
        return true;
      })
    : allRecords;
  ...
  const sinyalEntries = Object.entries(sinyalArsiv)
      ...
      <div className="bg-t-card rounded-xl overflow-hidden mb-5" style={{ border: "1px solid var(--bdr)" }}>
        ...
        {sinyalLoading ? (
          ...
          <div>
            ...
            <div className="grid grid-cols-7 gap-3 mb-4 px-4 pt-4">
          {[
            { id: 'toplam', icon: '📊', label: 'Toplam', count: stats.toplam, color: 'var(--txt3)' },
            { id: 'hedef', icon: '✅', label: 'Hedef', count: stats.hedef, color: 'var(--green)' },
            { id: 'stop', icon: '🔴', label: 'Stop', count: stats.stop, color: '#ef4444' },
            { id: 'acik', icon: '📂', label: 'Açık', count: stats.acik, color: 'var(--blue)' },
            { id: 'kar', icon: '🛡️', label: 'Kâr', count: stats.kar, color: '#2CC98A' },
            { id: 'risk', icon: '⚠️', label: 'Risk', count: stats.risk, color: '#F59E0B' },
            { id: 'basari', icon: '🎯', label: 'Başarı', count: basariOrani + '%', subcount: stats.hedef + '/' + stats.toplam, color: '#10b981', noFilter: true },
          ].map(s => (
            <button
              key={s.id}
              onClick={() => !s.noFilter && setActiveFilter(activeFilter === s.id ? null : s.id)}
              className="p-3 rounded-lg transition-all"
              style={{
                background: activeFilter === s.id ? 'var(--bg4)' : 'var(--bg3)',
                border: `2px solid ${activeFilter === s.id ? s.color : 'var(--bdr)'}`,
              }}
            >
              <div className="text-2xl mb-1">{s.icon}</div>
              <div className="text-3xl font-bold font-syne mb-1" style={{ color: s.color }}>{s.count}</div>
              {s.subcount && <div className="text-[10px] text-t-txt3 mb-1">{s.subcount}</div>}
              <div className="text-xs text-t-txt3">{s.label}</div>
            </button>
          ))}
        </div>
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {sinyalEntries.map(([key, rec]) => {
              ...
              const isTrailing = rec.durum === 'TRAILING STOP';
              const overlayVisible = (isHedef || isStop || isTrailing) && !gizliOverlay.has(key);
                ...
                <div key={key} className="bg-t-bg3 rounded-xl overflow-hidden" style={{ border: "1px solid var(--bdr)", position: 'relative' }}>
                  <div className="p-[12px_16px] flex items-center justify-between flex-wrap gap-2" style={{ borderBottom: "1px solid var(--bdr)" }}>
                    <div className="flex items-center gap-2">
                      ...
                      {rec.risk_bolgesi_aktif === true && rec.durum === 'AÇIK' && (
                        <span style={{fontSize:9, color:'#F59E0B', fontWeight:700, background:'rgba(245,158,11,0.15)', padding:'2px 5px', borderRadius:3, marginLeft:4}}>⚠️ RİSK BÖLGESİ</span>
                      )}
                  ...
                  {overlayVisible && isHedef && (() => {
                    ...
                    let kapSure = 0;
                    try {
                      const [d, m, y] = rec.tarih.split('.').map(Number);
                      const giris = new Date(y, m - 1, d);
                      const sonucDateStr = (rec.sonuc_tarih || '').split(' ')[0];
                      const [d2, m2, y2] = sonucDateStr.split('.').map(Number);
                      const son = new Date(y2, m2 - 1, d2);
                      kapSure = Math.max(0, Math.floor((son.getTime() - giris.getTime()) / 86400000));
                    } catch {}
                    return (
                    <div style={{
                      ...
                      <div style={{ textAlign: 'center', padding: '6px 0 5px', borderBottom: '1px solid rgba(44,201,138,0.3)', background: 'rgba(44,201,138,0.06)' }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: '#2CC98A', letterSpacing: 2 }}>🏆 {rec.hisse}</span>
                      </div>
                      <div style={{ display: 'flex', padding: '8px 14px 12px', gap: 14, flex: 1, minHeight: 0, overflow: 'hidden' }}>
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5, paddingRight: 12, borderRight: '1px solid rgba(44,201,138,0.3)' }}>
                        <div style={{ fontSize: 12, color: '#2CC98A', fontWeight: 700, letterSpacing: 1 }}>KAPANIŞ</div>
                        <div style={{ fontSize: 12, color: '#2CC98A' }}>Giriş: <b>{rec.giris.toFixed(2)} ₺</b> · {rec.tarih}</div>
                        <div style={{ fontSize: 12, color: '#2CC98A' }}>Hedef: <b>{rec.hedef.toFixed(2)} ₺</b> · {sonucTarih}</div>
                        <div style={{ fontSize: 13, color: '#2CC98A', fontWeight: 700, marginTop: 2 }}>+{kapPnlPct.toFixed(1)}% / +{kapPnlTL.toFixed(2)} ₺ · {kapSure} gün</div>
                      </div>
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5, paddingLeft: 4 }}>
                        <div style={{ fontSize: 12, color: '#2CC98A', fontWeight: 700, letterSpacing: 1 }}>📊 GÜNCEL</div>
                        <div style={{ fontSize: 12, color: '#2CC98A' }}>Fiyat: <b>{currentPrice.toFixed(2)} ₺</b></div>
                        <div style={{ fontSize: 12, color: 'rgba(44,201,138,0.85)' }}>Giriş: {rec.giris.toFixed(2)} ₺ · {rec.tarih}</div>
                        <div style={{ fontSize: 13, color: pnlPct >= 0 ? '#2CC98A' : '#E05252', fontWeight: 700, marginTop: 2 }}>{pnlPct >= 0 ? '+' : ''}{pnlPct.toFixed(1)}% / {pnlTL >= 0 ? '+' : ''}{pnlTL.toFixed(2)} ₺ · {gunFarki} gün</div>
                      </div>
                      </div>
                    </div>
                    );
                  ...
                  {overlayVisible && isStop && (() => {
                    ...
                    <div style={{
                      ...
                      <div style={{ textAlign: 'center', padding: '6px 0 5px', borderBottom: '1px solid rgba(224,82,82,0.3)', background: 'rgba(224,82,82,0.06)' }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: '#E05252', letterSpacing: 2 }}>💀 {rec.hisse}</span>
                      </div>
                      <div style={{ display: 'flex', padding: '8px 14px 12px', gap: 14, flex: 1, minHeight: 0, overflow: 'hidden' }}>
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5, paddingRight: 12, borderRight: '1px solid rgba(224,82,82,0.3)' }}>
                        <div style={{ fontSize: 12, color: '#E05252', fontWeight: 700, letterSpacing: 1 }}>KAPANIŞ</div>
                        <div style={{ fontSize: 12, color: '#E05252' }}>Giriş: <b>{rec.giris.toFixed(2)} ₺</b> · {rec.tarih}</div>
                        <div style={{ fontSize: 12, color: '#E05252' }}>Stop: <b>{rec.stop.toFixed(2)} ₺</b> · {sonucTarih}</div>
                        <div style={{ fontSize: 13, color: '#E05252', fontWeight: 700, marginTop: 2 }}>{kapPnlPct.toFixed(1)}% / {kapPnlTL.toFixed(2)} ₺ · {kapSure} gün</div>
                      </div>
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5, paddingLeft: 4 }}>
                        <div style={{ fontSize: 12, color: '#E05252', fontWeight: 700, letterSpacing: 1 }}>📊 GÜNCEL</div>
                        <div style={{ fontSize: 12, color: '#E05252' }}>Fiyat: <b>{currentPrice.toFixed(2)} ₺</b></div>
                        <div style={{ fontSize: 12, color: 'rgba(224,82,82,0.85)' }}>Giriş: {rec.giris.toFixed(2)} ₺ · {rec.tarih}</div>
                        <div style={{ fontSize: 13, color: pnlPct >= 0 ? '#2CC98A' : '#E05252', fontWeight: 700, marginTop: 2 }}>{pnlPct >= 0 ? '+' : ''}{pnlPct.toFixed(1)}% / {pnlTL >= 0 ? '+' : ''}{pnlTL.toFixed(2)} ₺ · {gunFarki} gün</div>
                      </div>
                      </div>
                    </div>
                  ...
                  {overlayVisible && isTrailing && (() => {
                    ...
                    <div style={{
                      ...
                      <button onClick={() => setGizliOverlay(prev => new Set(prev).add(key))}
                        style={{ position: 'absolute', top: 8, right: 10, background: 'none', border: 'none', color: 'rgba(44,201,138,0.6)', fontSize: 18, cursor: 'pointer', zIndex: 31, lineHeight: 1 }}>✕</button>
                      <div style={{ textAlign: 'center', padding: '6px 0 5px', borderBottom: '1px solid rgba(44,201,138,0.3)', background: 'rgba(44,201,138,0.06)' }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: '#2CC98A', letterSpacing: 2 }}>🛡️ {rec.hisse}</span>
                      </div>
                      <div style={{ display: 'flex', padding: '8px 14px 12px', gap: 14, flex: 1, minHeight: 0, overflow: 'hidden' }}>
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5, paddingRight: 12, borderRight: '1px solid rgba(44,201,138,0.3)' }}>
                        <div style={{ fontSize: 12, color: '#2CC98A', fontWeight: 700, letterSpacing: 1 }}>KAPANIŞ</div>
                        <div style={{ fontSize: 11, color: 'rgba(44,201,138,0.9)', marginTop: -4 }}>Trailing — Kâr Korundu</div>
                        <div style={{ fontSize: 12, color: '#2CC98A' }}>Giriş: <b>{rec.giris.toFixed(2)} ₺</b> · {rec.tarih}</div>
                        <div style={{ fontSize: 12, color: '#2CC98A' }}>Stop: <b>{rec.stop.toFixed(2)} ₺</b> · {sonucTarih}</div>
                        <div style={{ fontSize: 13, color: '#2CC98A', fontWeight: 700, marginTop: 2 }}>{kapPnlPct >= 0 ? '+' : ''}{kapPnlPct.toFixed(1)}% / {kapPnlTL >= 0 ? '+' : ''}{kapPnlTL.toFixed(2)} ₺ · {kapSure} gün</div>
                      </div>
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5, paddingLeft: 4 }}>
                        <div style={{ fontSize: 12, color: '#2CC98A', fontWeight: 700, letterSpacing: 1 }}>📊 GÜNCEL</div>
                        <div style={{ fontSize: 12, color: '#2CC98A' }}>Fiyat: <b>{currentPrice.toFixed(2)} ₺</b></div>
