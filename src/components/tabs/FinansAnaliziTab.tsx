...
export default function FinansAnaliziTab() {
        ...
        <div className="p-3 flex items-center justify-between" style={{ borderBottom: "0.5px solid #1e2535" }}>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(() => {
              ...
              const girisP = item.close > 0 ? Number(item.close) : 0;
              if (liveP > 0) {
                const degisim = girisP > 0 ? ((liveP - girisP) / girisP) * 100 : 0;
                const degisimTL = liveP - girisP;
                return (
                  <>
                    {girisP > 0 && (
                      <span className="text-[10px]" style={{ color: "#64748b" }}>Giriş: {girisP.toFixed(2)}₺</span>
                    )}
                    <span className="text-[12px] font-bold" style={{ color: "#fff" }}>{liveP.toFixed(2)}₺</span>
                    {girisP > 0 && (
                      <span className="text-[11px] font-bold" style={{ color: degisim >= 0 ? "#2CC98A" : "#E05252" }}>
                        {degisim >= 0 ? "▲" : "▼"}{Math.abs(degisim).toFixed(1)}% / {degisimTL >= 0 ? "+" : ""}{degisimTL.toFixed(2)}₺
                      </span>
                    )}
                  </>
                );
  ...
  return (
    <div className="animate-fade-in">
      ...
      {addTicker && (() => {
        const stock = snapData[addTicker];
        const liveP = livePrices[addTicker] && livePrices[addTicker] > 0 ? livePrices[addTicker] : (stock?.close ?? 0);
        return (
          <AddToPortfolioModal
            ticker={addTicker}
            price={liveP}
